...
init_root :: proc(
    ...
    root.root = root
    root.input.mouse.repeat_duration = mouse_repeat_duration
    root.input.mouse.repeat_movement_tolerance = mouse_repeat_movement_tolerance
    root.input = {}
...
input_mouse_press :: proc(root: ^Root, position: Vec2, button: Mouse_Button) {
    ...
    previous_mouse_repeat_tick := root.input.mouse.repeat_tick
    root.input.mouse.repeat_tick, tick_available = get_tick(root)

    if tick_available {
        delta := time.tick_diff(previous_mouse_repeat_tick, root.input.mouse.repeat_tick)
        if delta <= root.input.mouse.repeat_duration {
            root.input.mouse.repeat_press_count += 1
        } else {
            root.input.mouse.repeat_press_count = 1
        }

        // This is just a simple x, y comparison, not true distance.
        movement := root.input.mouse.position - root.input.mouse.repeat_start_position
        if abs(movement.x) > root.input.mouse.repeat_movement_tolerance ||
           abs(movement.y) > root.input.mouse.repeat_movement_tolerance {
            root.input.mouse.repeat_press_count = 1
        }
    }

    if root.input.mouse.repeat_press_count == 1 {
        root.input.mouse.repeat_start_position = root.input.mouse.position
    }
    ...
    send_global_event(root, Mouse_Repeat_Event{
        position = position,
        button = button,
        press_count = root.input.mouse.repeat_press_count,
    })
    if root.hover != nil {
