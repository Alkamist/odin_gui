...
_backend_vtable_tick_now :: proc(ctx: ^Context) -> (tick: Tick, ok: bool) {
    if ctx.vtable.tick_now == nil do return {}, false
    return ctx.vtable.tick_now()
}

_backend_vtable_set_mouse_cursor_style :: proc(ctx: ^Context, style: Mouse_Cursor_Style) -> (ok: bool) {
    if ctx.vtable.set_mouse_cursor_style == nil do return false
    return ctx.vtable.set_mouse_cursor_style(style)
}

_backend_vtable_get_clipboard :: proc(ctx: ^Context) -> (data: string, ok: bool) {
    if ctx.vtable.get_clipboard == nil do return "", false
    return ctx.vtable.get_clipboard()
}

_backend_vtable_set_clipboard :: proc(ctx: ^Context, data: string) -> (ok: bool) {
...
_window_vtable_font_metrics :: proc(window: ^Window, font: Font) -> (metrics: Font_Metrics, ok: bool) {
    if window.vtable == nil do return {}, false
    if window.vtable.font_metrics == nil do return {}, false
    return window.vtable.font_metrics(window, font)
}

_window_vtable_render_draw_command :: proc(window: ^Window, command: Draw_Command) {
    if window.vtable == nil do return
